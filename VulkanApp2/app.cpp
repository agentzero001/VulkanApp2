...
void Application::createLogicalDevice() {
	...
	//Specifying a bunch of details in structs. With VkDeviceCreateInfo containing the structs 
	//VkDeviceQueueCreateInfo and VkPhysicalDeviceFeatures
...
void Application::createGraphicsPipeline() {
	...
	rasterizer.rasterizerDiscardEnable = VK_FALSE; //disables the output to the framebuffer
	rasterizer.polygonMode = VK_POLYGON_MODE_FILL; //any mode other than fill requires enabling a GPU feature
...
void Application::createTextureImage() {
	...
	transitionImageLayout(textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
		copyBufferToImage(stagingBuffer, textureImage, static_cast<uint32_t>(texWidth), static_cast<uint32_t>(texHeight), graphicsQueue, device, commandPool);
...
void Application::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
	...
	//note that the order of your clear values should be identical to the order of your attachments.
...
void Application::drawFrame() {
	...
	//these are variables which can be changed or updated at runtime
	updateUniformBuffer(currentFrame);

	vkResetFences(device, 1, &inFlightFences[currentFrame]);
	vkResetCommandBuffer(commandBuffers[currentFrame], 0);

	//Record a command buffer which draws the scene onto the acquired swapchain image.
...
VkShaderModule Application::createShaderModule(const std::vector<char>& code, VkDevice& device) {
...
void Application::updateUniformBuffer(uint32_t currentImage) {
	...
	ubo.model = glm::rotate(glm::mat4(1.0f), 4 * time * glm::radians(10.0f), glm::vec3(0.0f, 0.0f, 1.0f));
